...
const initializeSampleData = () => {
  // Sample users
  users.set('7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU', {
    id: 1,
  ...
  // Set initial timers for active auctions
...
function sendAuctionState(auctionId, socket = null) {
  ...
  const auctionState = {
    auction,
    bids: auctionBids,
    participantCount: Math.floor(Math.random() * 15) + 5, // Simulated
    remainingTime
  };
...
async function handleNewBid(data, socket) {
  ...
  if (timer) {
    ...
    auctionTimers.set(parseInt(auctionId), {
      remaining: auction.timer_seconds,
      lastUpdate: Date.now()
    });
  }
  ...
  auctionTimers.forEach((timer, auctionId) => {
    ...
    if (timer.remaining === 0) {
      ...
      if (auction && auction.status === 'active') {
        ...
        // Restart auction after 5 seconds for demo purposes
        setTimeout(() => {
...
app.post('/api/users', (req, res) => {
  ...
  if (!user) {
    user = {
      ...
      sol_balance: '10.0000', // Give users some initial balance for testing
      usdt_balance: '5000.00',
      total_bids: 0,
      total_won: 0,
      created_at: new Date().toISOString(),
...
server.listen(PORT, () => {
  console.log(`🚀 Crypto Auction House (Simple Mode) running on port ${PORT}`);
  console.log(`📊 Using in-memory storage for quick demo`);
  console.log(`⚡ Real-time bidding ready`);
  console.log(`💰 Users get 10 SOL + 5000 USDT for testing`);
});
